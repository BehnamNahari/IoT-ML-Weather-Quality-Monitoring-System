...
static void CLI_Execute(char *cmd)
{
    ...
    {
        CLI_Send("Commands: help status aqi alarm history forecast fault\r\n");
    }
    ...
    {
        snprintf(out, sizeof(out),"Alarm=%s  Level=%d  Count=%u\r\n",Alarm_IsActive() ? "ON" : "OFF",(int)Alarm_GetLevel(),Alarm_GetCounter());
        CLI_Send(out);
    }
