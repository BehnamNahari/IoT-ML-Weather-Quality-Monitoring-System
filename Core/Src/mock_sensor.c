 ...
 *
 * نحوه کارکرد:
 *  - هر پارامتر یک مقدار پایه (base) دارد که آرام آرام drift می‌کند
 *  - نویز کوچک روی هر نمونه اضافه می‌شود (شبیه سنسور واقعی)
 *  - در سناریوی POLLUTED مقادیر CO2 و PM2.5 به سمت آستانه‌های خطر می‌روند
 *
 * رنج‌های فیزیکی:
 *   Temperature : 15 .. 35  C
 *   Humidity    : 20 .. 90  %RH
...
/* ------------------------------------------------------------------ */
/*  متغیرهای داخلی (static)                                            */
/* ------------------------------------------------------------------ */
static uint32_t       s_seed;
static MockScenario_t s_scenario;

/* مقادیر پایه که آرام drift می‌کنند */
...
/**
 * @brief عدد float بین [lo, hi) برمی‌گرداند
 */
static float RandFloat(float lo, float hi)
{
    /* NextRand را به بازه [0, 1) نگاشت می‌کند */
    float t = (float)(NextRand() & 0xFFFFu) / 65536.0f;
    return lo + t * (hi - lo);
}

/**
 * @brief یک مقدار base را آرام به سمت target می‌برد (drift)
 *        rate: حداکثر تغییر در هر نمونه
 */
...
/**
 * @brief مقدار را به بازه [min_val, max_val] محدود می‌کند (clamp)
 */
...
{
    /* --- 1. target را بر اساس سناریو تعیین کن --- */
    float target_pm25;
    float target_co2;
    float target_temp;
    ...
    switch (s_scenario)
    {
        case MOCK_SCENARIO_POLLUTED:
            /* CO2 و PM2.5 به سمت آستانه‌های خطر می‌روند */
            target_pm25 = 180.0f;
            target_co2  = 1600.0f;
    ...
    /* --- 2. پایه‌ها را به سمت target drift کن --- */
    ...
    /* --- 3. نویز کوچک اضافه کن و داده را clamp کن --- */
    gWeather.temperature = Clamp(
        s_temp_base + RandFloat(-TEMP_NOISE,  TEMP_NOISE),
        15.0f, 45.0f);

    gWeather.humidity = Clamp(
        s_hum_base  + RandFloat(-HUM_NOISE,   HUM_NOISE),
        0.0f, 100.0f);

    gWeather.pressure = Clamp(
        s_pres_base + RandFloat(-PRES_NOISE,  PRES_NOISE),
        950.0f, 1060.0f);

    gWeather.pm25 = Clamp(
        s_pm25_base + RandFloat(-PM25_NOISE,  PM25_NOISE),
        0.0f, 500.0f);

    gWeather.co2 = Clamp(
        s_co2_base  + RandFloat(-CO2_NOISE,   CO2_NOISE),
        300.0f, 5000.0f);
}
